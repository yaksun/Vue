...
Vue.directive('drag', {
  bind:function (el) {
    console.log(el);
    let documentEl: any = el.querySelector('.drog-box');
    documentEl.style.cursor = 'move';
    // 监听document是因为如果监听元素el的话鼠标太快移出元素后就会失效
    documentEl.onmousedown = (event: any) => {
      //算出鼠标相对元素的位置
      ...
      document.onmousemove = (e)=>{
        ...
        } else if ( left > document.documentElement.clientWidth - el.offsetWidth/2) {
          left = document.documentElement.clientWidth - el.offsetWidth/2
        }
        
        if(top - el.offsetHeight/2 <= 0) {
          top = el.offsetHeight/2;
        } else if ( top > document.documentElement.clientHeight - el.offsetHeight/2) {
          top = document.documentElement.clientHeight - el.offsetHeight/2;
        }
